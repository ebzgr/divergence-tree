...
DEPRECATED: This script is kept for backward compatibility. Please use `analyze_results.py` instead.
Both scripts have the same functionality, but `analyze_results.py` is the canonical version.
...
def analyze_results(
    ...
    simulation_type : str, default="auto"
        Type of simulation: "lambda_comparison", "method_comparison", or "auto" (auto-detect).
    use_tuning : bool, default=True
    ...
    if simulation_type == "auto":
        if any("twostep" in m for m in available_methods):
    ...
    if simulation_type == "lambda_comparison":
        method_labels = {
