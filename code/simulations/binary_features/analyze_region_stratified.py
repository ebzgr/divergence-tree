...
def _metric_col(prefix: str, metric: str) -> str:
    """Map (method prefix, metric) to the correct dataframe column name."""
    if metric == "cpu_time" and prefix.startswith("twostep_"):
        return f"{prefix}_total_cpu_time"
    ...
    for factor_name, factor_info in factor_list:
        ...
        for metric in metrics:
            ...
            for factor_val in factor_values:
                ...
                # Keep the row only if at least one method had values for this metric/bin.
                if not any(k.endswith("_mean") for k in row_data.keys()):
                    continue
            ...
            for prefix, label, color, marker in method_configs:
                ...
                if mean_key not in factor_data[0]:
                    continue
            ...
            for _, method_label, _, _ in method_configs:
                mean_k = f"{method_label}_mean"
                std_k = f"{method_label}_std"
                n_k = f"{method_label}_n"
                if mean_k not in factor_data[0]:
                    continue
                metric_payload["series"][method_label] = {
                    "n": [int(d[n_k]) for d in factor_data],
                    "mean": [_round5(d[mean_k]) for d in factor_data],
                    "std": [_round5(d[std_k]) for d in factor_data],
                }
