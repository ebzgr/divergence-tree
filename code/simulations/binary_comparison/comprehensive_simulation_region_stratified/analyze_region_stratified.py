...
def _metric_bad_threshold(metric: str) -> Tuple[str, float]:
    """
    Return (direction, threshold) for "very bad" outcomes.

    direction:
      - "low": values <= threshold are very bad
      - "high": values >= threshold are very bad
    """
    # These are intentionally simple, interpretable defaults.
    if metric in ("accuracy", "precision_region_2", "recall_region_2", "f1_region_2"):
        return "low", 0.55
    if metric == "fnr_region_2":
        return "high", 0.45
    if metric in ("runtime", "cpu_time"):
        return "high", float("nan")  # computed as p95 per-method
    if metric == "n_leaves":
        return "high", float("nan")  # computed as p95 per-method
...
def create_diagnostics_tables(
    ...
    *,
    ...
    """
    Produce additional "health" statistics from all simulation rows:

    1) Tail-rate/quantiles per method × metric (how often results are "very bad").
    2) Factor-bin variability table (where results are unusually unpredictable).

    Outputs are written into output_dir as CSV files.
    """
    utils.safe_makedirs(output_dir)

    # ------------------------------------------------------------------
    # 1) Tail rates & quantiles per method × metric
    # ------------------------------------------------------------------
    ...
    for _, prefix, method_label in _method_configs():
        for metric in metrics:
            ...
            if metric in ("runtime", "cpu_time", "n_leaves") or (isinstance(thr, float) and np.isnan(thr)):
                # Data-driven "bad" threshold for non-[0,1] metrics or unknown scales:
                # top 5% is "very bad" by default.
    ...
    # ------------------------------------------------------------------
    # 2) Scenario unpredictability: high within-bin spread (std / IQR)
    # ------------------------------------------------------------------
    factor_defs = {
        ...
        "dispersion": {"column": "dispersion", "is_discrete": False, "log_x": True},
        "rareness": {"column": "rareness", "is_discrete": False, "log_x": True},
    }
    method_plot_configs = [
        ("divtree_lambda0", "λ=0"),
    ...
    if not var_df.empty:
        # Flag "unpredictable" bins as those in the top decile of std per (factor, metric, method).
        var_df["std_p90"] = var_df.groupby(["factor", "metric", "method"])["std"].transform(lambda s: float(np.quantile(s, 0.9)))
...
def create_factor_comparison_plots(df: pd.DataFrame, output_dir: str, *, verbose: bool = True) -> None:
    """
    Factor-wise line plots: aggregate simulations into bins (same idea as
    comprehensive_simulation/analyze_lambda_comparison.py). Continuous factors
    use binning; discrete sparsity uses exact levels. Log-spaced bins and a
    logarithmic x-axis are used for factors that are log-uniform or bounded
    lognormal in the DGP (noise, data_size, dispersion, rareness).
    """
    utils.safe_makedirs(output_dir)
    ...
    factors = {
        ...
        "dispersion": {
            "column": "dispersion",
            "label": "Dispersion (region-2 aspect)",
            "xlabel": "Dispersion",
            "is_discrete": False,
            # Bounded lognormal in DGP (simulation_base._sample_bounded_lognormal)
            "log_x": True,
        },
        "rareness": {
            "column": "rareness",
    ...
    total_factor_plots = sum(
        1
        for fname, finfo in factor_list
        for _ in metrics
    ...
    for factor_name, factor_info in factor_list:
        ...
        for metric in metrics:
            done_factor += 1
            _log(
                f"  [factor plots] {done_factor}/{total_factor_plots}: "
                f"{factor_name} × {metric} ...",
                verbose,
            )
...
def analyze_region_stratified(
    ...
    *,
    ...
    _log(
        f"  Loaded {len(df):,} rows × {len(df.columns)} columns in {time.perf_counter() - t0:.1f}s",
        verbose,
    )
