"""
DGP diagnostics for region-stratified simulation (v4).
"""
...
def analyze_dgp(
    ...
    *,
    ...
    aspect_cols = ["noise", "data_size", "sparsity", "rareness", "intensity"]
    for col in aspect_cols:
        if col in df.columns:
            _save_hist(
                ...
                log_x=(col in ("noise", "data_size", "intensity")),
            )

    # Realized region share distributions over simulations.
    for r in [1, 2, 3, 4]:
        col = f"rareness_real_region{r}"
        if col in df.columns:
            _save_hist(
                df[col],
                title=f"Distribution of {col}",
    ...
    x_col, y_col = "rareness_target_region2", "rareness_real_region2"
    ...
    summary_cols = [
        c
        for c in df.columns
        if c in aspect_cols or c.startswith("rareness_") or c.startswith("combo_share_real_region")
    ]
    if summary_cols:
        df[summary_cols].describe(include="all").T.to_csv(os.path.join(output_dir, "dgp_summary_statistics.csv"))

    # Outcome histogram from first N simulations.
    base_output_dir = os.path.dirname(os.path.dirname(os.path.dirname(results_file)))
    data_root = os.path.join(base_output_dir, "data", "v3_lambda_twostep_comparison")
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(base_dir, "aggregated", "v4_lambda_twostep_comparison", "analysis", "dgp_diagnostics")
