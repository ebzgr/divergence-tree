"""
Base utilities for region-stratified comprehensive simulation v4.

v4 differs from v3 mainly in *how aspects are enumerated* (grid, not random).
Method wrappers remain the same as v3 (including TwoStep holdout tuning for step 2).
"""
...
def _explicit_n_categories_for_sparsity(k: int) -> list[int]:
    """
    Explicit category allocations for the targeted V4 sparsity settings.
    ...
    if k == 10:
        return [3] * 10
    raise ValueError(
        f"Unsupported sparsity={k} for v4. Expected one of {sorted(config.SPARSITY_GRID)}."
    )
...
def run_twostep_method(
    ...
    result: Dict[str, Any] = {}
    try:
        # --- 1) Tune causal forest hyperparameters on outcome 1 (YF) using the validation set ---
        ...
        # --- 2) Fit both causal forests on TRAIN only using best params (reuse for YC) ---
        # and generate region labels for train/val to tune the classification tree.
        ...
        # --- 3) After tuning, refit forests on TRAIN+VAL and fit final classifiers ---
        X_tv = np.concatenate([X_train, X_val], axis=0)
        T_tv = np.concatenate([T_train, T_val], axis=0)
        YF_tv = np.concatenate([YF_train, YF_val], axis=0)
        YC_tv = np.concatenate([YC_train, YC_val], axis=0)
